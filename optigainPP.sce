...
//  7      Nov 2016    Add lines to step plots for Perf Specs.
...
////  Generate Argument Vectors
Kpa = kmin:dk:kmax;
Kia = kimin:dki:kimax;
Kda = kdmin:dkd:kdmax;

function [a,b,c,d,e,f]=cost2(Kp,Ki,Kd,pp)
    [a,b,c,d,e,f]=costPID(plant, Kp,Ki,Kd,pp)
endfunction

[ts1r, po1r, ssr, cur,gmr, yr] = parallel_run( Kpa, Kia, Kda,pp, "cost2");

n = 0
//Loop through and get best for each 
for Kp = kmin:dk:kmax,
    for Ki = kimin:dki:kimax,
        for Kd = kdmin:dkd:kdmax,
            n = n + 1  // get index for results
